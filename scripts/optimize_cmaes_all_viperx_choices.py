...
from t2.robogen.viperx import VARIABLE_DOF_COMPONENTS
...
if __name__ == "__main__":
    ...
    parser.add_argument("--orn_noise", type=float, default=0.0)
    ...
    choices = enumerate_choices(VARIABLE_DOF_COMPONENTS)
    ...
    for choice, num_uniforms in sorted(choices, key=lambda x: str(x[0])):
        ...
        cmd = [
            "python",
            "scripts/optimize_cmaes_viperx.py",
            "runner=mink",
            "env@runner.env=variable_viperx",
            "robogen_from_params_fn._target_=t2.robogen.viperx.variable_dof_viperx_full_variation_from_params",
